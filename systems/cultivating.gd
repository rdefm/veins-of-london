...
# R§3.4: growth 0-19 -> 1, ..., 100+ -> 6 (a wildCeiling vein past 100 still reads as 6 -- "1..6" is a hard ceiling, not extrapolation).
...
# Shared by every surface a vein appears on (map sheet, station bubble, vein list) so the wording never drifts between them.
...
# Concrete plain-numbers phrasing (CONTENT-GUIDE.md §4), shared by the map sheet, station bubble, and vein list.
...
# Tier drives ore yield directly (R§1.2 terroirYieldMult): poor 0.6 / fair 1.0 / rich 1.6 / saturated 2.4.
...
static func make_vein(ore_type: String, growth: int, district: String, site_id: Variant, hospitability: Dictionary) -> Dictionary:
	return {
		...
		"growth": growth,
		"security": "none",
		# Purchased alarm-upgrade ids, mirroring state.home["security"]'s shape -- independent of the "security" tier ladder above (R§1.6).
		"alarmUpgrades": [],
...
# at/below neutral is allowed (it just yields 0, per prune_yield above); only time-block affordability gates the button.
...
# depth is the caller's choice of pruneLightDepth (9) or pruneHardDepth (24), R§1.2. No cultivating XP awarded -- only cultivate() awards it.
...
# Called from TimeSystem.daily_tick() step ④ (R§3.1). Order matters: drift, then
...
# R§3.4: a player vein sat at its ceiling for rampantSeedDays consecutive ticks
# spawns a fresh vein on a random unclaimed site in the same district. Claims an
# existing site (siteCap untouched); competing with the player's own prospecting for unclaimed sites is intentional, not a bug.
static func self_seed(vein: Dictionary) -> void:
	...
	# No unclaimed site to seed into: rampantDays holds at the threshold and retries next tick.
...
# the specific growth transitions the growth-gauge glyph cares about; never
# re-fires while a vein merely sits in a band. Called from every place growth
# actually changes: _drift_one above (band-boundary crossings) and
# cultivate()/prune() (neutral-line crossings, which drift alone can never
# cause). Burst and drain are mutually exclusive by construction -- burst on a growth increase, drain on a decrease -- so a single call never queues both.
static func _queue_growth_events(vein: Dictionary, growth_before: int) -> void:
	...
	# Both bounds inclusive: a vein already sitting exactly at neutral that gets pruned further down still counts as crossing below neutral.
	var neutral: int = GameData.VEIN_GROWTH["neutral"]
...
# Factions.rivalry_success_chance) reads defensive strength through here rather
# than indexing GameData.VEIN_SECURITY directly. extraGuards defaults to 0 via .get() for veins that predate it.
...
# Home.add_security. Independent of upgrade_vein_security above -- a vein's "security" tier and its "alarmUpgrades" array are separate purchases.
