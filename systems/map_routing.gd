...
# "Crosses" here means "runs closer than line_clearance to", not a literal
# geometric intersection -- two different-faction lines may touch a shared
# interchange stop but shouldn't run alongside each other with no gap.
...
# Unlike the river (a polyline crossing is a hard line), a stop is a filled
# circle on the rendered map -- "crosses" means the leg passes within the
# stop's own drawn radius, not just an exact geometric crossing.
...
# Which obstacle_stops a two-leg elbow (a->corner->b) actually crosses --
# used once diag-first/diag-last has run out of orientations, to find which
# stop(s) still need a position nudge (MapCanvas._apply_crossing_nudges).
...
# How many leading points two polylines share (exact match).
# nearest_neighbour_order() is a deterministic greedy walk, so re-running
# it with one extra stop reproduces every earlier pick until the new stop
# first out-competes whatever would otherwise be picked next --
# grow_segment() below uses this to find where the lines diverge.
...
# The polyline segment that "grows in" when one more stop (`new_stop`)
# joins an owner's existing line (`old_stops`) -- the actual tail of
# build_line()'s recomputed output once the new stop is included, so an
# animated hand-off ends in exactly the shape the static draw produces at
# rest. Degenerates cleanly when `old_stops` is empty: common_prefix_length
# is then 0, so the segment is simply the new line in full.
