...
#
# GameData.SITE_TIER_ORDER (data/sites.json) is the single source of truth for
# worst-to-best ordering -- don't duplicate it as a local const.

# tierIndex for the NPC-claim formula (distinct from SITE_TIER_ORDER's
# worst-to-best position): poor 0, fair 1, rich 2, saturated 3. barren is excluded -- it is never claimed.
const NPC_CLAIM_TIER_INDEX: Dictionary = { "poor": 0, "fair": 1, "rich": 2, "saturated": 3 }

# Faction veins now only die via Cultivating.collapse_vein()'s left-wall roll
# (adr/0004), so a freshly-claimed vein's expected survival is longer than
# adr/0002's original curve assumed; these are tuned down from that curve to
# keep the claim rate proportional -- needs balance sign-off once played, not derived from a hard target vein count.
...
# The stable, permanent slot a stop keeps for its whole life, stamped once at
# creation onto the site (or, for a saturated site's extra natural-vein stop,
# onto the vein itself -- attempt_seed() below) rather than derived from
# where the stop currently sits in state.world.sites/player.veins. Drains
# state.world.mapSlotFreePool before minting a fresh value off the per-district
# mapSlotCounters, so a freed slot recirculates instead of counting up forever.
...
# The other half of next_slot_index() -- called by every removal path that ends
# a site's or a slotIndex-owning vein's existence in a district
# (_reroll_worst_unclaimed(), Cultivating.collapse_vein(), VeinTrade.sell_to_faction(), Raiding.resolve_raid_outcome()'s claim branch).
...
# Used when the caller has a vein id (combat["veinId"]) but not the faction that
# owns it -- unlike find_site() above, this searches every site's factionVein regardless of owner.
...
# "This faction's own live site veins" -- shared so the buy-side lane (the Trade
# modal's Assets rows, its cart gather, VeinTrade.buy_from_faction()) all agree on one definition of "buyable from this faction".
...
# Shared by compute_tier_weights() and compute_at_cap_tier_weights(): floats out
# a tier-keyed weight table (GameData.SITE_TIER_WEIGHTS or
...
# cultivating skill (rich +2*(skill-1), saturated +1*(skill-1), barren
# -3*(skill-1) floor 5). Kept separate from GameState reads so tests can hit the floors directly without faking a whole district.
...
# Weighted roll over GameData.SITE_TIER_ORDER (normalisation is implicit in the cumulative-weight walk).
...
static func roll_tier(district_id: String) -> String:
	...
	# busker_greenwich (M1-LONDON D5): a one-shot +10 rich weight tip-off, consumed on the very next Greenwich prospect regardless of outcome.
...
# Once a district is at siteCap, the site being rolled replaces land that's
# already been picked over, so it draws from GameData.SITE_AT_CAP_TIER_WEIGHTS
# instead -- a fixed, heavily poor/barren-weighted table with no
# siteQualityMod/skill/tip-off inputs (those describe the district's land, not its picked-over state).
...
# oreBias weights are the listed types' probabilities; the remainder is split uniformly among the other ore types (uniform oreBias = 0.2 each).
...
# rich -> exactly one bonus, uniformly; saturated -> all three, plus a 5%
# chance of hasNaturalVein. barren/poor/fair get nothing.
...
static func prospect(district_id: String) -> Dictionary:
	...
	# Checked before advance_time_block(), not after: advance_time_block() can itself
	...
	# The story beat is checked first -- it's Rng-free, so a genuine early return
	# never touches the deck's own seeded roll. DistrictDeck.maybe_trigger() must stay the last thing either branch does.
...
# Shared by _worst_unclaimed_site() and best_unclaimed_site(): every
# truly-unclaimed site in the district, sorted tier-first (direction per
...
# Deletes the district's worst truly-unclaimed site and rolls a fresh one in its
...
# (Factions._seed_day_one_vein()) can fabricate a from-scratch site with the same
# tier/ore/bonus generation a real prospect uses, without _create_site()'s
# player-action side effects (state append, discover animation, prospect XP).
...
# Debug-only site creation from the Debug phone app: district, tier and oreType
# are player-chosen rather than rolled, and this bypasses the district's
# siteCap entirely -- a testing tool, not a simulated prospect. Same site shape
# as roll_new_site() (and debug_start.gd's _debug_site()) minus the ore-type/bonus rolls.
...
static func attempt_seed(site_id: String) -> Dictionary:
	...
	# Dial.attempt_seed()'s mixed five-ore-type cost (R§3.5), which has nothing for attunement to match against.
	...
	if success:
		...
		# vein of the site's oreType at the same seedGrowth, with its own freshly-rolled (not shared) location.
		...
		if site["hasNaturalVein"]:
			...
			# Second stop landing on an already-positioned site (the first vein reuses the site's own slotIndex) -- needs its own permanent slot.
...
# Faction veins only die via Cultivating.collapse_vein()'s left-wall roll, same as player veins (adr/0004).
...
# The district's best (highest-tier, oldest-breaks-ties) unclaimed site, used by rival_prospector (M1-LONDON §D5 #13), or null if none.
...
# events.gd's _faction_seed_reported_sites (col_a1_des_report's on_complete):
# create the vein at seedGrowth, attach it to the site, queue its map events. Callers own the "should this site get a vein" gating.
...
# rival_prospector's "refuse to pay" outcome: faction-claims the district's best
# unclaimed site outright, instant vein and all -- a deterministic consequence,
# not the daily-tick's probabilistic claiming (WHICH faction claims it still
# goes through the same weighted pick_claimant() the daily tick uses). No-op if the district has no unclaimed site to claim.
...
# Called from TimeSystem.daily_tick() step ⑤b. Each unclaimed, non-barren site
# may attract a faction claim; older, richer sites are likelier. The claimant is
# always one of the 5 canonical factions (Factions.pick_claimant()), and claiming instantly seeds a real vein.
