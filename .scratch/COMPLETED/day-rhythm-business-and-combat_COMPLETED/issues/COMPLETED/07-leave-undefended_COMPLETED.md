...
- [x] Implement the approved timing from ticket 06 alongside Go and defend and Decide later, with three clearly distinct responses.
- [x] Explain the affected vein, deadline and consequence before commitment; closing/back remains deferral.
- [x] Revalidate eligibility and make repeat/stale responses harmless; apply consequences exactly once at the approved time.
- [x] Persist the approved non-intervention state and render badges/warnings according to the approved lifecycle.
- [x] Test commitment, cancellation/back, expiry, duplicate response and save/load; device QA verifies that deferral cannot accidentally abandon defence.
