...
- [x] Delegation is an exclusive Sales automation assignment per contract —
  a delegated contract is not manually deliverable, and vice versa.
- [x] Sales has no delivery-cap limit; it handles every delegated contract in
  the player-set priority order from ticket 25.
- [x] After every increase in shared stock (crafting, Procurement, or
...
- [x] At daily rollover, Sales also makes partial deliveries from available
  shared stock in priority order.
- [x] Sales earns 5 XP for sourcing an offer (wire this here since it's the
