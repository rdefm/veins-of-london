class_name SmsArchie2Screen
extends Control

# SMS thread 2 (ARCHIE_SMS_2): lines up the buyer. Staged reveal per T12's
# deferral note — await get_tree().create_timer(...).timeout in the
# screen, 0.6/0.9s alternating delays. The reveal counter is screen-local
# presentation state, not game state.
