...
const GLYPHS := {
 ...
 "✓": ["00000", "00001", "00001", "10010", "01010", "00100", "00000"],
 "▣": ["11111", "10001", "10101", "11111", "10101", "10001", "11111"],
 "□": ["11111", "10001", "10001", "10001", "10001", "10001", "11111"],
	...
	";": ["00000", "01100", "01100", "00000", "00100", "01000", "00000"],
...
const _BLANK := ["00000", "00000", "00000", "00000", "00000", "00000", "00000"]


static func rows_for(ch: String) -> Array:
	if ch.is_empty():
		return _BLANK
...
static func draw_char(target: Object, origin: Vector2, ch: String, dot_size: float, colour: Color, dim_colour: Color) -> void:
	...
	for row_index in GLYPH_H:
		...
		for col_index in GLYPH_W:
			var lit := row[col_index] == "1"
			var pos := origin + Vector2(col_index, row_index) * dot_size
...
static func text_width(text: String, dot_size: float, char_gap: float) -> float:
	if text.is_empty():
		return 0.0
	return text.length() * (GLYPH_W * dot_size) + (text.length() - 1) * char_gap
