...
| dot_matrix_board.gd | field-kit-chrome ticket 02: shared electronic dot-matrix departure/platform board renderer (amber-on-black, custom `_draw()` against dot_matrix_font.gd's bitmap table, scramble-on-refresh transition). `top_bar.gd`'s status line and each `notification_toast.gd` row mount their own single-line instance of this so the two read as one continuous board; ticket 03's post-combat outcome log (`combat.gd`'s `_build_log()`) is a second, multi-line consumer, clip-wrapped the same way a notification row is |
