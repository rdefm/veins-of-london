...
| dot_matrix_board.gd | field-kit-chrome ticket 02: shared electronic dot-matrix departure/platform board renderer (amber-on-black, custom `_draw()` against dot_matrix_font.gd's bitmap table, scramble-on-refresh transition). `top_bar.gd`'s status line and each `notification_toast.gd` row mount their own single-line instance of this so the two read as one continuous board; ticket 03 (combat log) is a planned second consumer, multi-line-capable already for that reason |
...
| notification_toast.gd | Auto-fading unseen-notification rows, rendered via dot_matrix_board.gd (field-kit-chrome ticket 02 — the merged status/notification board; ranked "1st"/"2nd" prefixes since the two visible rows share that board with top_bar.gd's status line) |
