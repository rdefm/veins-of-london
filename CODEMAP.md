...
| notify.gd | Notifications-list append/evict helpers; `META_COMBAT_LOG` meta-flag constant (ticket 03) marks an entry as combat-log-sourced for notification_toast.gd's suppression check | — |
...
| File | Purpose |
|---|---|
| app_tile.gd | Icon+label+badge+lock tile used by the phone app grid. field-kit-chrome ticket 04: no longer shared with the dock — `nav_bar.gd` now builds its own dock-local tile chrome (Family 4's TfL-style strip vs. this component's Family 2 cream-frame look) |
| bag_drawer.gd | Global bottom-sheet bag drawer, openable from any screen |
| contact_cards.gd | Shared Archie/James/faction contact-card builders. 08-family-2-chrome-contacts: also owns `apply_phone_os_chrome(root)`, a recursive repaint pass (card panel fill, button fill/outline, label ink/muted colour, all via `GameData.PALETTE`'s new `phone_os` group) that `contacts.gd` runs over its built screen tree — deliberately not wired into `build_faction_card()` below, which still serves the untouched (cream) Factions screen |
...
| dot_matrix_board.gd | field-kit-chrome ticket 02: shared electronic dot-matrix departure/platform board renderer (amber-on-black, custom `_draw()` against dot_matrix_font.gd's bitmap table, scramble-on-refresh transition). `top_bar.gd`'s status line and each `notification_toast.gd` row mount their own single-line instance of this so the two read as one continuous board. (ui-chrome-pass ticket 02: `combat.gd`'s post-fight outcome-log consumer, `_build_log()`, is removed — the mid-fight ticker already covers that line-by-line, so this board is single-line-only again) |
...
| notification_toast.gd | Auto-fading unseen-notification rows, rendered via dot_matrix_board.gd (field-kit-chrome ticket 02 — the merged status/notification board; ranked "1st"/"2nd" prefixes since the two visible rows share that board with top_bar.gd's status line). Ticket 03: combat-suppression (hold everything while `state.combat.active`) is narrowed — entries stamped `Notify.META_COMBAT_LOG` (the mid-fight ticker lines `combat.gd`'s `_on_beat_played()` posts) bypass the hold and render live, competing for the same `MAX_VISIBLE` slots as any other source |
...
| top_bar.gd | Persistent top bar: cash, day/time-blocks, bag button, rendered as the larger top row of the dot_matrix_board.gd board shared with notification_toast.gd (field-kit-chrome ticket 02). Unconditionally visible except title/intro (Main.gd's TOP_BAR_HIDDEN_SCREENS) |
