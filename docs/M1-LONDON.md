...
**NPC site-claiming (daily tick, step ⑤b):** each unclaimed site: `p = 0.02 + 0.01 × tierIndex + 0.005 × ageDays` (tierIndex: poor 0, fair 1, rich 2, saturated 3; barren never claimed; ageDays = day − discoveredDay), cap 0.15. Claimed → one of the 5 canonical factions is picked (`Factions.pick_claimant()`, heavily weighted toward the district's `factionPresence`) and instantly seeded a real vein at `growth = seedGrowth` (20) (`site.factionVein`, oreType from the site, security tier rolled from faction flavour/vein value/faction resource level — see faction-vein-ownership T01), notification "<Faction> have moved onto the <tier> site in <district>." Faction-claimed sites remain visible on the map; taking one back is M2 combat content — in M1 they persist until they die via growth-collapse-at-zero (below), same as any vein. (Retuned down again in bugfixes-73/adr/0004, from 0.03/0.02/0.01/cap-0.25, alongside removing NPC-abandonment below — see that entry.)

**NPC abandonment — REMOVED (bugfixes-73/adr/0004).** This section originally specified a second, independent daily-tick step (⑤c, right after ⑤b): each faction-claimed site rolled `p = 0.02 + 0.005 × ageDaysSinceClaim` (cap 0.08, retuned once already by ticket 22 from an even faster 0.05/0.01/0.15 curve), deleting the site and its embedded faction vein outright on a hit. Both retunings still churned faction veins fast enough to visibly trend toward zero, so the mechanic itself was removed rather than tuned a third time: a faction vein now dies **only** via the same growth-collapse-at-zero roll (`collapseChancePerDay`, 0.15/day once pinned at 0 — §1.2/vein-growth-state spec §2.5) every player vein already faces — deleting the site outright on that branch, same end effect ("wait out the good faction-claimed site" is still never viable), just one roll instead of two stacked. Step ⑤c is now `Sites.roll_faction_vein_growth()` (prune-back, below) — see adr/0004 for the full reasoning and `systems/time_system.gd`'s daily_tick() for the current step order.
...
`state.world.currentDistrict`. Acting on a district-located action (prospect, seed, cultivate, prune) sets currentDistrict to that action's district as a free side effect — there is no travel surcharge; every districted action costs the same block count regardless of whether it targets the current district or a different one (show the cost in the button label as just the action's own cost: "Prune (light) — 1 block"). The standalone Map tab "Travel" button switches currentDistrict for free (0 blocks) and, like prospecting, rolls for a D5 district event on completion. Waking up (daily tick / rest) resets currentDistrict to "shoreditch" (home). Selling uses the district you are in at sale time (priceMod + dangerMod on the mug roll: `getEffectiveMugChance(0.20 + dangerMod)`).
...
New event `archie_cultivation` slots into the tutorial right after the home-raid debrief (trigger: first visit to the Map tab after `archiePartnerSeen`). Archie walks the player through cultivating his transferred Whitechapel time vein: 6–9 cards teaching growth/drift → Cultivate → Prune (light/hard) → the collapse risk of leaving a spent vein alone, ending with effects: force one free successful cultivate on that vein (growth up by `cultivate_gain`, no block cost — special effect op `tutorial_cultivate`, vein-growth-state ticket 10), archie relation +2, flag `cultivationTutorialSeen`. Prose: DRAFT, PROSE-REVIEW; Archie voice per tone bible.
...
4. NPC-claim and NPC-abandonment curves behave correctly under soak: no district ever permanently locks out prospecting, and an abandoned site's slot reliably rerolls fresh on the district's next prospect. (NPC-abandonment itself was later removed — bugfixes-73/adr/0004; the same soak now drives the freed-slot path through growth-collapse-at-zero instead, see test_playthrough.gd's m1_20_seed_soak case.)
