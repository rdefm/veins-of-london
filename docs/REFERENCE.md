...
state = {
  ...
  sales: { pendingOffers:[], activeContracts:[], nextOfferId:1, nextContractId:1 }, # ticket 24: pure sales ledger. An offer stores its source/template/request/expiry and immutable quote {unitValue, liveValue, payment, salesSkill}; acceptance creates a stable-id contract with its own dueDay. Delivery/settlement fields are extended by later business tickets.
