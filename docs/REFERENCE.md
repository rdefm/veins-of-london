...
- **Staff block step:** every working cultivator and producer (room hire or founder role) acts at the end of every player time block — `TimeSystem.advance_time_block()` runs `run_staff_block()` for the block that just ended, before any rollover; `TimeSystem.do_rest()` runs it once per block still remaining that day, then rolls over, so a day always yields 3 actions per worker. Order within a block: cultivators, then producers, then (if shared stock grew) Sales' instant delivery re-check via `shared_stock_increased`. A worker acts only if `Payroll.is_working(id)` (a room hire once today's wage is paid; founders always). Block output (ore yielded, items crafted, net ore change) accumulates in `morningAccounts.blockProduction` and folds into the next rollover's Morning Brief `production`/`oreMovement`. The rollover staff phase keeps only wages, Sales partial deliveries, settlements/renewals and offer sourcing.
