...
- **Left wall (collapse):** `growth` pins at 0. Each daily tick a vein sits at 0, `chance(collapseChancePerDay)` (0.15) removes it: a player vein's site reverts to unclaimed; a faction vein's site is deleted outright. Still cultivable (a full gain roll — no diminishing-near-zero rule anymore) while pinned at 0. This is the only way a faction vein dies — see the NPC-claim/faction-vein-growth bullet in §3.1.
