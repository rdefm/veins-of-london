...
**DRAFT — pending sign-off (ticket 01, combat-refining, 2026-09-19).** Three amendments to the `combat` dict above, needed before combat-refining tickets 03/05/07/10 can build against it. Additive/renaming only — no change to any formula, XP award, reward, or flee/item rule.
...
- **Resumable turn progression** (**DRAFT — pending sign-off**, ticket 01 combat-refining, 2026-09-19): today `Combat.player_attack()` walks the whole of `build_turn_queue()` in one synchronous call, resolving every entry (including every Motion-inserted extra player slot) before returning. This amendment makes progression resumable, so a player decision point actually pauses the engine rather than only pausing an already-fully-resolved round's animation:
  - `combat.turnCursor` (§2 amendment) tracks position within the current round's `queue`.
  - `Combat.advance_to_next_decision()` (draft name) replaces `player_attack()`'s old whole-round loop as the engine's resume function: starting from `turnCursor.index`, it auto-resolves every non-`"player"` entry in place (ally/enemy turns, unchanged bodies — `_ally_turn`/`_enemy_turn`; a koed entry is skipped exactly as today) and stops the instant either `combat.outcome` becomes non-null (fight over, no decision point) or a `"player"`-type entry is reached — original round slot or a Motion-inserted extra slot alike, both ordinary queue entries now. It never resolves the player entry itself; it parks there and waits.
  - A player command (Attack / Item / Run / a Dial cast) resolves **exactly the one queued player-type entry the cursor is parked on** — never a whole round — then calls `advance_to_next_decision()` again to run forward to the next pause. This is the behavioural change from today: a Motion round's 2nd/3rd hit is no longer auto-resolved inside the same call as the 1st — each is its own decision, and the player may choose differently each time (e.g. Attack then Item).
  - **Round boundary:** when `turnCursor.index` reaches `queue.size()` with no outcome yet, `advance_to_next_decision()` runs the round-boundary tick (unchanged from today's end-of-`player_attack()` decrement — `motionTurns -= 1` and its expiry log line; no other per-round timer changes), then rebuilds `queue` via `build_turn_queue(combat)` against the post-tick state, resets `index = 0`, increments `round`, and continues auto-resolving into the new round exactly as above.
