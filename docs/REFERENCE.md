...
vein-growth-state PRD: a vein has one signed condition axis, `growth: int` (0..ceiling, default ceiling 100, neutral 50), replacing the old `devBar`/`level`/`charged`/`chargeBlocks` quartet entirely. cultivation-refining ticket 01 adds a second, persistent axis alongside it: `level: int` (1..`level_cap_for_tier(tier)`, seeded at 1 on every fresh vein — see the terroir cap table below). Left alone, growth drifts nightly away from neutral (50) at a magnitude driven by the vein's level; the player pushes it back with Cultivate (right) or Prune (left).

Bands (`Cultivating.growth_band`/`_band_for_growth`) are label-only groupings for UI (map bubble, vein list, station bubble, the burst/drain map-event thresholds) — they no longer drive drift, which is level-based (below):

| band id | growth range | label |
|---|---|---|
| collapsed | 0 | Spent |
| barren | 1–14 | Barren |
| sparse | 15–29 | Sparse |
| thinning | 30–44 | Thinning |
| dormant | 45–55 | Dormant |
| taking | 56–70 | Taking |
| lush | 71–85 | Lush |
| wild | 86–99 | Wild |
| rampant | 100 (ceiling) | Rampant |

**Terroir level cap** (`levelCapByTerroir`, `Cultivating.level_cap_for_tier`/`level_cap`): a site's terroir tier caps how far its vein's earned level can rise — `poor: 2, fair: 3, rich: 4, saturated: 5`. Every fresh vein (`Cultivating.make_vein`) seeds at level 1 regardless of tier. Nothing yet raises or lowers level past its seed value — that lands in later cultivation-refining tickets (05/06/07).

**Drift** (`Cultivating.drift_magnitude`/`_drift_one`, R§8.3): exactly 50 is the sole stable point — a vein sitting there does not drift. Every other nightly tick rerolls `drift_magnitude = level + rand(driftRandomMin, driftRandomMax)` (`driftRandomMin: 1, driftRandomMax: 5`, so 2-6/day at level 1) and steps growth that far further from 50, toward whichever wall it already leans. The old 45–55 no-drift band and the old per-band drift-rate table are gone — bands are labels only now (above).

Other constants (`data/vein_growth.json`): `yieldPerPoint: 0.35`, `hardPruneBonus: 1.25`, `pruneLightDepth: 9`, `pruneHardDepth: 24`, `cultivateGainMinOffset: 5`, `cultivateGainMaxOffset: 9`, `collapseChancePerDay: 0.15`, `seedGrowth: 20`, `rampantSeedDays: 5`, `selfSeedGrowth: 60`, `wildCeilingBonus: 20`, `terroirYieldMult: { poor: 0.6, fair: 1.0, rich: 1.6, saturated: 2.4 }`, `levelCapByTerroir: { poor: 2, fair: 3, rich: 4, saturated: 5 }`, `driftRandomMin: 1`, `driftRandomMax: 5`, `levelYieldMultBase: 1.0`, `levelYieldMultPerLevel: 0.2`, `developmentThreshold: 90`, `levelUpChancePerDay: 0.10`.

**Cultivate** (`Cultivating.cultivate`, cultivation-refining ticket 03): no success/fail roll — every call applies a positive whole-number `growth` gain, uniformly rolled `[skill+cultivateGainMinOffset, skill+cultivateGainMaxOffset]` (= `[skill+5, skill+9]`), applied in full and clamped only by remaining headroom to `ceiling(vein)` — a ceiling-limited gain smaller than the nominal minimum is not a failure. Skill 1's minimum (6) is sized to exactly cover a level-1 vein's worst-case nightly drift (`1 + driftRandomMax = 6`, R§8.3), so one cultivate action a day permits maintenance even on repeated minimum rolls. A seated Movement's matching-ore attunement bonus (R§3.5) multiplies the rolled gain by `(1 + bonus)` instead of shifting a chance, since there's no chance left to shift. Flat 15 XP per action.

**Prune** (`Cultivating.prune(vein_id, depth)`, light -9 / hard -24): yield counts only growth points removed from above neutral — `points = max(0, growth_before-50) - max(0, growth_after-50)`, `yield = round(points * yieldPerPoint * terroir_yield_mult(vein) * level_yield_mult(vein) * hardBonus)` (hardBonus 1.25 for a hard prune, 1.0 for light), then `apply_yield_bonus`. `level_yield_mult(vein) = levelYieldMultBase + levelYieldMultPerLevel * (level-1)` (cultivation-refining ticket 04: 1.0x at level 1 up to 1.8x at level 5), layered on top of terroir's own multiplier. Pruning at or below neutral always yields 0. `Cultivating.prune_resulting_growth(vein, depth) = max(0, growth - depth)` is the same growth math as a pure, non-mutating preview (used by prune() itself and by ticket 10's harvest chooser) — a hard harvest doesn't always drop a vein out of the development zone (e.g. a wildCeiling vein at 120 hard-pruned to 96 stays >= developmentThreshold).

**Development streak** (`developmentStreak: int`, cultivation-refining ticket 04/05): a per-vein counter of consecutive nights held at/above `developmentThreshold` (90) with no in-between dip, seeded at 0 by `Cultivating.make_vein`, that ticket 06's level-up roll reads. `Cultivating._clear_streak_below_threshold(vein)` is the single shared invariant hook — called after every place `growth` actually changes (`cultivate()`, `prune()`, `_drift_one()`) — that zeroes `developmentStreak` the instant post-mutation growth drops below 90, even a same-day dip the player later cultivates back above it. It only clears; ticket 05 owns incrementing it on an eligible night.

**Level-up roll** (`Cultivating._roll_level_up`, cultivation-refining ticket 06): called from `drift_veins()` right after that night's streak advance and before `_drift_one()`, for every eligible vein (`is_development_eligible`, above). `chance = min(1.0, levelUpChancePerDay * (developmentStreak-1))` — day 1 of eligibility (streak 1) always rolls 0%, day 2 rolls 10%, ..., capped at 100%. On success: `level` += 1 (bounded to `level_cap(vein)`), `growth` resets to exactly `neutral` (50), `developmentStreak` clears to 0, and the roll's return value tells `drift_veins()` to skip that vein's `_drift_one()` call for the night — no drift layers on top of the reset. A vein already at its terroir's level cap is never eligible (`is_development_eligible` excludes it), so it never rolls at all; it can still sit above `developmentThreshold` indefinitely for harvest/raid purposes.

**Left wall**: `growth` pins at 0 (never negative). A vein above level 1 that hits 0 depletes instead of risking the collapse roll (`Cultivating.collapse_vein`, cultivation-refining ticket 07): `level` -= 1, `growth` resets to exactly `neutral` (50), `developmentStreak` clears via the usual hook — one level lost per depletion event, never a cascade, and no collapse roll fires that same tick. Applies to player and faction veins alike. A level-1 vein at 0 keeps the original behaviour below: each daily tick, `chance(collapseChancePerDay)` removes the vein — a player vein's site reverts to unclaimed (re-seedable); a faction vein's site is deleted outright. This is the ONLY way a faction vein dies (bugfixes-73/adr/0004 removed the separate NPC-abandonment roll adr/0002 originally added — see §3.1's faction-vein-claim/growth bullet below).
...
**Complications** — loading (`Dial.load_complication(recipeKey, tier)`) moves one unit of any existing crafted consumable recipe out of `Crafting`'s tier-bucketed `player.inventory` (§2/§3.5) into `player.dial.loadedComplications`, unchanged in tier; unloading (`Dial.unload_complication(index)`) reverses it exactly — never duplicated or destroyed. Each loaded entry is `{ recipeKey, tier, detent }`: `detent` is a cosmetic display-order position for the (out-of-scope) Collar UI, assigned as the entry's load-time index, read nowhere else. Ticket 14 dropped the old per-recipe `capacityCost` field (`data/recipes.json` no longer has one) — every loaded entry now costs exactly one slot, regardless of which recipe it is or the tier it was crafted at. `Dial.capacity_used(dial)` (count of loaded entries) may never exceed `dial.capacityMax`.

`Dial.capacity_max(level) `: `capacityByLevel = [0, 1, 2, 3, 4, 4]` (index = Dial level) — grows only with Dial level, never with which Movement (if any) is seated, kept structurally independent of the loadout choice.
...
state = {
  ...
  productionLog: [],  # staff producers' record, oldest day first: [{day, blocks:[{block (0-2 time-block index), entries:[{contactId, made:{recipeKey:{"<tier>":count}}, failed:{recipeKey:count}, oreShort:null|{recipeKey, ore:[oreType]}}]}]}]; Rooms.process_staff_block() appends one block record per block with any producer attempt or ore-short stop (oreShort = first below-target recipe the crafter couldn't afford, and which ore types fell short; null when it stopped with every target met); rollover (daily_tick ⑥.2) drops days older than constants.json productionLogDays (10) counting the current day; old saves backfill []
  businessStats: { today:{revenue:0, expenses:0, oreCultivator:0, orePlayer:0}, days:[] }, # BizBrief Stats tab (§3.10 "Business stats"). today = running tally since the last rollover; days = [{day, revenue, expenses, oreCultivator, orePlayer, items}], oldest first, trimmed to constants.json businessStatsDays (10); old saves backfill empty
  morningAccounts: { latest:null, autoOpenedDay:0, blockProduction:{ore:{},items:{},oreMovement:{}} }, # blockProduction = today's staff block output, folded into the next rollover's account (§3.10 "Staff block step"); # latest = exact completed-rollover account {day, openingBalance, closingBalance, income, expenses, oreMovement, production:{ore,items}, sales, losses:{ore,veins}, exceptions}; exceptions are plain dicts by `kind`: missedJob{recipeKey}, productionShortfall{recipeKey,target,actual}, loss{source}, arrearsInterest{amount}, arrearsShortfall{amount,arrears}, forcedDowngrade{fromTier,toTier,roomsLost,arrearsCleared,arrears}, arrearsCountdown{arrears,tier,interestInDays?,downgradeInDays?} (absent key = already compounding / bedsit), wageShortfall{contactId,amount}; latest.payday = that rollover's business ledger record or null; autoOpenedDay is the once-only presentation receipt.
  sellState: {},              # sell-menu qty selections, transient
  sales: { pendingOffers:[], activeContracts:[], nextOfferId:1, nextContractId:1 }, # ticket 24: pure sales ledger. An offer stores its source/template/request/expiry and immutable quote {unitValue, liveValue, payment, salesSkill}; acceptance creates a stable-id contract with its own dueDay. Delivery/settlement fields are extended by later business tickets. Each contract carries `periodStartDay`, `playerAssisted`, `delegatedWholePeriod`; each settlement carries `qualified: bool` (§3.10 "Unattended proof").
  business: { potActive:false, pot:0, week:{startDay, receipts, expenses:[{kind:"wage"|"calc", contactId?, contractId?, source?, amount}]}, partners:[], wages:{}, ledger:[], nextPaydayId:1 }, # §3.10 "Business pot and payday". wages[contactId] = {weekly, owed, unpaid, hiredDay, daysWorked, promptPending}; ledger[] = {payday:"payday-N", day, receipts, expenses, shares:{player, <partnerId>...}}.
...
`enemies` in combat (§3.7a — supersedes the old single `enemy` field): array of up to 3 `{ name, hp, hpMax, attackMin, attackMax, speed:int, weapon, ability, evadeChance, koed:bool, variant? }` — `variant` (string) is set only on a Territorial Scrapper: the territorial sprite-set key (`GameData.TERRITORIAL_VARIANTS`, or `"default"` when the pool is empty) rolled at spawn, see §3.7a Roster generation. `veinId` moved up to sit directly on `combat` (one vein per fight regardless of guard count), not per-enemy; store `veinId`, not an object reference (state purity). `combat.focusedEnemyIndex` addresses the player's current single-target into this array — see §3.7a.
...
**Signed off and implemented (ticket 03, combat-refining, 2026-09-19): `combat.turnCursor` below. Signed off (ticket 05, combat-refining, 2026-09-21): `combat.selection` below.** Signed off (ticket 10, combat-refining, 2026-09-23): `combat.locationKey` below. Additive/renaming only — no change to any formula, XP award, reward, or flee/item rule.

- **`combat.turnCursor: { queue: Array, index: int, round: int }`** — the resumable progression cursor (§3.7a "Resumable turn progression" amendment). `queue` is this round's turn order, same entry shape `build_turn_queue()` already returns (`{type, index, speed}`, `extra:true` on a Motion-inserted slot); rebuilt only at a round boundary. `index` is how many of `queue`'s entries have resolved so far (0-based; `index == queue.size()` means the round is over and a rebuild is due). `round` is a 1-based counter, informational only. Set fresh (`{queue: [], index: 0, round: 0}`; the first `advance_to_next_decision()` call populates it) by `_start_combat()`; never carried between fights.
- **`combat.selection: { type: String, index: int }`** (`type`: `"player" | "ally" | "enemy"`) replaces `combat.focusedEnemyIndex: int`. No save-migration concern — combat state is fight-scoped, reset fresh by `_start_combat()` every time, never restored across saves the way `player`/`world` are. `index` is meaningless when `type == "player"` (always 0). KO-clamp: if the selected entry becomes koed (an ally dies, the focused enemy dies), auto-clamp — same-type first (next living ally/enemy in array order), else fall back to `{type:"enemy", index:0}` (the pre-existing "next living enemy" default, §3.7a Targeting). Auto-clamp never fires for `type:"player"` (the player selection is never invalid mid-fight — a player loss ends combat outright, §3.7). An enemy-only action (Attack, Blast, Time Pearl, Black Hole) is simply **unavailable** — greyed/rejected, `{ok:false, reason:"Select an enemy first."}` — whenever `selection.type != "enemy"`; it never silently redirects to a fallback enemy (§3.7a Targeting amendment, §3.7 ally-effect table).
- **`combat.locationKey: String`** — recorded once at `_start_combat()`, immutable for the fight. Derivation by `context`: `CONTEXT_RAID`/`CONTEXT_DEFEND_VEIN`/`CONTEXT_EVENT_RAID` → the fought vein's `district` (§2.1, via `veinId`); `CONTEXT_MUGGING`/`CONTEXT_EVENT_MUGGING`/`CONTEXT_ARCHIE_DEAL_MUGGING` → `world.currentDistrict` at combat start; `CONTEXT_HOME_RAID`/`CONTEXT_HOME_ALARM_DEFEND` → the fixed key `"home"` (home isn't a district). Backdrop lookup order using this key: `docs/combat-animation-vision.md` §2.1.
...
**Placement chronology:** ticket 11's historical paragraph below describes
its original grid. The 2026-09-21 phone-home refresh after the Alarms
paragraph is current and supersedes that placement, not its mechanics.
...
Day-rhythm ticket 03 adds **BizBrief** (`bizbrief`) to that grid. Each completed daily tick stores one exact Morning Brief: resulting day, Reynard's opening/closing balance and actual ledger income/expense totals, actual stock/production/loss/exception effects. It does not duplicate transaction rows. Attention is derived live from unresolved raid records and unread messages. A matching Evening→Morning/Rest presentation opens it once after animation completion; load/reopen cannot rerun processing or replay the presentation.

Day-rhythm ticket 04 adds **Alarms** (`alarms`) to the phone grid. Its badge and stable summary rows derive live from `home.pendingRaid` and `world.pendingDefendRaids`; no alarm state is duplicated. Rows show target, district, end-of-day intervention deadline, and the already-rolled claim/loot consequence. Defend revalidates through `Home.trigger_defend()` / `Raiding.trigger_defend()`; Decide later and Back only navigate away. BizBrief Attention routes to Alarms. While unresolved, one grouped raid-alarm line is pinned on the dot-matrix board ahead of routine notifications; the board remains informational and has no buttons.

**Phone-home refresh (2026-09-21; supersedes earlier launcher placement in this section, not app mechanics):** the framed simulated phone home uses a four-column launcher in this exact order: **Alarms** (`alarms`), **Notes** (`notes`), **BizBrief** (`bizbrief`), **The Ticker** (`ticker`), **Factions** (`factions`), **Reynard's** (`bank`), **Harrow's** (`property`), **My File** (`profile`), **Contacts** (`contacts`), **TfL** (`vfl`), **Notifications** (`notifications`), **Save/Load** (`saveload`). **Debug** (`debug`) is absent on normal games and appended after Save/Load only when `flags.debugStartUsed` is true. TfL keeps its existing Archie-partner Map gate and direct Map routing; Save/Load, Debug, every other app's mechanics, and existing routes are unchanged.

The simulated home dock contains exactly **Phone** (`dialer`), **Messages** (`messages`), and **Settings** (`settings`), symmetrically spaced on one translucent/dark rounded surface. Phone is a no-mechanics recent-calls placeholder. Messages opens the conversation index and then the existing thread views, and carries total unread count. Settings owns the existing reduced-motion and alarm-vibration preferences, moved out of My File/Profile. Messages and Settings appear only in this simulated dock, never in the main launcher. This dock is inside the phone fiction and is distinct from the unchanged external **Phone / Map / HQ** navigation; the external Phone tab remains the home-navigation button. The persistent amber top board also remains unchanged outside the simulated device.

The home status time/date/weather/location/connectivity/battery display is fixed decorative mockup data, not state or mechanics: it adds no calendar, weather, connectivity, or battery fields to `GameState` or saves, and performs no host-clock lookup or network request. Numeric badges derive live from their existing sources rather than adding saved counts. All phone-tab art lives under `assets/phone/`: the wallpaper at `assets/phone/phone-wallpaper.jpg`, runtime launcher icons at `res://assets/phone/icons/<app_id>.png` (square 128×128 alpha PNG), and larger supplied images in `assets/phone/source/` as source artwork for normalisation only.

**Leave undefended** (day-rhythm ticket 06, approved): choosing it is a deliberate immediate non-intervention for that one pending raid, not a deferral. It first opens a confirmation that identifies the target, says the raid resolves now, and states the already-rolled claim/loot consequence; cancelling, closing, or backing out is Decide later and changes nothing. Confirming consumes only that live pending record and runs the same expiry resolution path immediately: guards receive their normal repel roll; if they repel it, nothing is lost, otherwise the recorded outcome resolves exactly once. The record has no additional `nonIntervention` state: the committed action is the resolution itself. Stale or repeated confirms revalidate the matching live record and harmlessly no-op if it is gone, expired, resolved, or otherwise ineligible. Other simultaneous alarms remain pending. Save/load preserves only still-pending records; it never restores a dismissed confirmation or repeats an immediate result. Existing alarm/security/defence eligibility is unchanged.
...
- **Time transition presentation:** TimeSystem emits source/destination once per block charge or Rest. A transient Main overlay waits for event/combat/result/modal/bag completion and a 0.75s outcome hold, then blocks input for 1.75s. The current screen stays visible behind a dim layer. A 220px circular London park scene and destination label rise, hold during the phase sky/sun/moon change, then exit below the screen. Early Rest traverses the remaining phase changes in order. Completion has no effects. Load/reset/Rewind state replacement discards presentation. `meta.reducedMotion` is an optional saved boolean (absent = false), set through Phone Settings; it holds the destination scene and label in place for the same duration. Art/tuning: `data/daily_cycle.json` and `assets/daily_cycle/park_foreground.png`.
...
- **daily_tick order (exact):** ① tick barometer ② roll home raid ③ home bill (ADR 0006): `BILL = round(base * (1 + fx.dailyCost))`, `base` = tier `dailyCost` (rent) if `home.tenure == "rented"`, else utilities; then, in order (ADR 0006 "Daily ordering"): (1) if `arrears > 0` and `arrearsDays ≥ interestThresholdDays`, `arrears += round(arrears × interestRate)` (never barometer-scaled); (2) pay arrears `min(cash, arrears)` (bank "Arrears"); (3) pay `min(cash, BILL)` (bank "Living costs"), remainder added to `arrears`; (4) `arrearsDays = 0` if `arrears == 0`, else `+= 1`; (5) at `arrearsDays ≥ downgradeThresholdDays` and not the bedsit, drop one tier via `Home.change_tier` into a rented home (owned tier lost → `arrears = 0`; rented → kept), `arrearsDays = 0`; the bedsit just keeps accruing. Notify amounts actually paid, interest, shortfall and balance owed (append " You are flat broke." if cash hits 0); a forced downgrade gets its own warning. Cash never goes negative ③b Healing Salve HoT (see §3.7) ③c passive HP regen (bugfixes-42): unconditional, always-on, independent of Rest and the Salve HoT (stacks with both) — `heal = round(hpMax * PASSIVE_REGEN_FRACTION)` (`PASSIVE_REGEN_FRACTION` = 0.05), capped at hpMax, skipped (no notification) if already at full HP; notification "You rest easy. +X HP." ④ vein growth drift (`Cultivating.drift_veins()`, §1.2/§8.3): for each player vein and each faction vein sitting off neutral, `delta = drift_magnitude(level) = level + rand(1,5)`, `direction = +1 if growth>50, -1 if growth<50`, `growth = clamp(growth + delta*direction, 0, ceiling(vein))` (a vein exactly at 50 does not drift); a vein pinned at 0 then rolls `collapseChancePerDay` (0.15) to be removed (site reverts to unclaimed for a player vein, deleted outright for a faction vein) ⑤ tutorial day-triggers (day ≥ 2 & stage "buyer_event" & !buyerEventSeen → queues ARCHIE_SMS_2's content as a real pendingMessages entry, once, guarded by `archieBuyerSmsQueued` — bugfixes-83, no separate notification; stage "archie_craft_chat" & day ≥ archieChatUnlockDay → notification "Archie wants to meet up. Check Contacts.") ⑤b NPC site-claiming ⑤c faction-vein prune-back ⑥ wages, Production shortfall check, Sales partial deliveries, settlements/renewals, business payday / owed-wage retry (§3.10 "Business pot and payday"), offer sourcing (staff work itself runs per block, §3.10) ⑦ Dial charge regen (`Dial.daily_regen()`, dial-device ticket 07 — replaces the old device system's per-device charge reset; §1.4/§3.5). (This line predates the faction-economy/rivalry/raiding steps that now run between ⑤c and ⑥ — see `systems/time_system.gd`'s own daily_tick() doc comment for the full, current step list; not re-derived here to avoid a second copy drifting out of sync.)
...
- `cultChance = min(0.90, 0.30 + (skill−1) * 0.12)` — still used by Seed's `seedSuccessChance` (below) and by `Rooms._cultivator_act()`'s contact cultivating; no longer used by the player's own Cultivate (see below, cultivation-refining ticket 03).
...
- **Cultivate(vein):** 1 block. No success/fail roll — `growth += cultivate_gain(skill, growth, ceiling(vein))`, where `cultivate_gain` uniformly rolls `[skill+5, skill+9]` inclusive (boosted by a seated Movement's matching-ore attunement bonus, R§3.5, as a `(1+bonus)` multiplier), clamped to the remaining headroom below `ceiling(vein)` — a ceiling-clamped gain smaller than the nominal minimum is not a failure. Flat 15 XP. Result modal always reports success.
- **Prune(vein, depth):** 1 block. `depth` is `pruneLightDepth` (9) or `pruneHardDepth` (24). `points = max(0, growth_before−50) − max(0, growth_after−50)` where `growth_after = max(0, growth_before − depth)`; `yield = round(points * yieldPerPoint(0.35) * terroirYieldMult(vein.hospitability.tier) * levelYieldMult(vein) * hardBonus)` (hardBonus 1.25 for a hard prune, 1.0 for light), then `apply_yield_bonus`. Pruning at or below neutral (50) always yields 0. No cultivating XP awarded (matches the harvest schedule this replaces). Also clears `developmentStreak` if `growth_after < developmentThreshold` (90) — see §1.2's development-streak entry.
- **Left wall (depletion/collapse):** `growth` pins at 0. A vein above level 1 depletes instead: `level` -= 1, `growth` resets to `neutral` (50), `developmentStreak` clears, one level per event, no cascade, no collapse roll that tick (cultivation-refining ticket 07). A level-1 vein at 0 keeps the original roll: each daily tick, `chance(collapseChancePerDay)` (0.15) removes it — a player vein's site reverts to unclaimed; a faction vein's site is deleted outright. Still cultivable (a full gain roll — no diminishing-near-zero rule anymore) while pinned at 0, which is how both are rescued before the reset/roll lands. This is the only way a faction vein dies — see the NPC-claim/faction-vein-growth bullet in §3.1.
...
- **Bench work is free of time:** crafting, probing and refining at the Lab bench never advance a time block, carry no time-cost label, and stay available when the day's blocks are spent. Ore costs and XP are unchanged.
...
- **Complications — load/unload:** `Dial.load_complication(recipeKey, tier)` moves one unit out of `player.inventory`'s tier bucket (§2/above) into `player.dial.loadedComplications` at a flat cost of one slot (§1.4 — every recipe costs the same one slot, regardless of tier), refused once `Dial.capacity_used()` would exceed `capacityMax`; `Dial.unload_complication(index)` reverses it exactly, unit intact.
...
- **Use Black Hole** (calc-effect-wiring-02): consume; deal `effectPower(skill)` damage to the enemy immediately (can win the fight outright); `frozenTurns += 1 + floor(effectPower(skill) / 8)`, added once per use (not per enemy hit — the pool already covers every enemy) — always additive, no reuse guard, stacks with Time Pearl or a prior Black Hole.
...
- **Ally-targetable effects** (**DRAFT — pending sign-off**, ticket 01 combat-refining, 2026-09-19): every "Use X" bullet above is self/enemy-targeted exactly as written unless listed here as ally-eligible. This table is exhaustive over the combat-usable consumable/Complication set — a future addition must extend it explicitly, never assume a self-effect transfers to an ally.

  | Effect | Ally-targetable | Behaviour when `combat.selection.type == "ally"` |
  |---|---|---|
  | Healing Burst | **Yes (confirmed)** | Heals the selected ally instead of the player: `ally.hp = min(ally.hp + effectPower(skill), ally.hpMax)`, same consume/log/beat shape as the self-cast today. Same cost as self-cast (1 unit) and the same single queued player-turn (§3.7a progression) — ally-targeting changes only who's healed, not what it costs or how many decision points it spends. |
  | Healing Salve | No | Out-of-combat only (above) — selection is irrelevant, it can't be used mid-fight regardless of target. |
  | Time Pearl | No | Enemy-only; unavailable unless `selection.type == "enemy"` (§2 amendment). |
  | Enhancement Powder | No | Self-only; the extra-turn buff is defined only for its caster. Granting an ally an extra queue entry is a distinct mechanic, not specified here. |
  | Blast | No | Enemy-only damage target; its self-only flee-boost side effect is unaffected by `selection`. |
  | Shield | No | Self-only buff (`player.shieldPool`) — the ticket's flagged example of a self-effect that does **not** transfer; an ally shield pool isn't specified. |
  | Black Hole | No | Enemy AoE — already ignores `selection` entirely. |
...
- **Resumable turn progression** (implemented, ticket 03 combat-refining, 2026-09-19): a player decision point actually pauses the engine rather than only pausing an already-fully-resolved round's animation:
  - `combat.turnCursor` (§2 amendment) tracks position within the current round's `queue`.
  - `Combat.advance_to_next_decision(combat, beats)` is the engine's resume function: starting from `turnCursor.index`, it auto-resolves every non-`"player"` entry in place (ally/enemy turns, unchanged bodies — `_ally_turn`/`_enemy_turn`; a koed entry is skipped exactly as its own turn body already handles) and stops the instant either `combat.outcome` becomes non-null (fight over, no decision point) or a `"player"`-type entry is reached — original round slot or a Motion-inserted extra slot alike, both ordinary queue entries now. It never resolves the player entry itself; it parks there and waits. `Combat.prime_decision_point(combat, beats)` (queue-empty check, only does real work on a fight's very first decision) and `Combat.conclude_decision_point(combat, beats)` (index++ then resume) wrap it around every command below.
  - A player command (Attack / Item / Run / a Dial cast) resolves **exactly the one queued player-type entry the cursor is parked on** — never a whole round — then runs `advance_to_next_decision()` again to reach the next pause. A Motion round's 2nd/3rd hit is its own decision, not auto-resolved inside the same call as the 1st, and the player may choose differently each time (e.g. Attack then Item). Item use and a Dial cast now spend the queued turn the same way Attack/Flee always did — they are no longer free actions outside the queue.
  - **Round boundary:** when `turnCursor.index` reaches `queue.size()` with no outcome yet, `advance_to_next_decision()` decrements `motionTurns` once (with its expiry log line) **only if the round that's ending actually carried a Motion-inserted `extra` slot** — activating Motion via an item mid-round sets `motionTurns` after that round's queue was already built without extras, so the bonus takes effect starting next round rather than being silently spent before it's ever used; no other per-round timer changes. It then rebuilds `queue` via `build_turn_queue(combat)` against the post-tick state, resets `index = 0`, increments `round`, and continues auto-resolving into the new round exactly as above.
  - "**Combatants faster than the player act before the first command of a round**" falls out of this directly: any ally/enemy entry sorted ahead of the player's slot in `queue` is auto-resolved by the very first `advance_to_next_decision()` call of the round, before the player is ever asked for input.
  - **Snapshot per decision point:** `push_combat_snapshot()` moves from "once per `player_attack()` call" to once per resolved player-type queue entry — called immediately before each one resolves, so a Motion round pushes 2–3 snapshots (one per hit) instead of today's 1. The existing bounded-stack cap of 2 (§3.9) is unchanged; Rewind still restores the oldest of whatever's on the stack, cursor included (`turnCursor` now travels with the snapshot/restore).
  - Everything else is unchanged: `build_turn_queue()`'s own sort/tie-break/motion-insertion logic, every per-turn resolution body, XP (`COMBAT_XP_PER_ATTACK_TURN`, still once per resolved player entry — a Motion round now visibly awards it 2–3 times, matching "once per player turn taken" as already written, not a formula change), rewards, flee/item rules, `exit_combat()`/`onWin` dispatch.
  - **Flagged, not resolved here:** since item use now spends the queued player turn, a defensive/CC item (Time Pearl, Black Hole's freeze, Shield, Prophet's Breath) used while a live enemy is still due its own turn this round has that turn auto-resolved immediately afterward by the same `advance_to_next_decision()` call — a 1-turn grant can read back as already spent by the time the item's own call returns, and a shield/freeze pool can be partly or fully drained by that same immediate enemy turn before the player gets to act on it. Mechanically consistent with the contract above, but a real playtest/balance question (does defensive item timing need its own rule?) this ticket doesn't answer.
...
- **Persistent selection, generalised** (**DRAFT — pending sign-off**, ticket 01 combat-refining, 2026-09-19): the "Targeting" bullet above is amended — `combat.focusedEnemyIndex` becomes `combat.selection: {type, index}` (§2 amendment), addressable at the player, an ally, or an enemy, not only an enemy. Swiping the turn-order strip still sets it (`Combat.set_focused_enemy()` becomes a generalised `Combat.set_selection(type, index)`, same non-mutating-outside-selection contract — no snapshot, no turn, no log). Auto-clamp-on-KO and the enemy-only-action-unavailable rule are specified in the §2 amendment, not repeated here. AoE (Black Hole) is unaffected — it already ignores `selection` entirely.

- **Bounded queue-projection policy** (**DRAFT — pending sign-off**, ticket 01 combat-refining, 2026-09-19): the turn-order strip (`docs/combat-animation-vision.md` §2.4) needs a "what's coming" read that is neither the bare unresolved tail of the current round (too short — doesn't guarantee every living combatant's next turn is visible) nor an unbounded future (pre-rolls outcomes that haven't happened). `Combat.project_queue(combat)` (draft name) is a pure, non-mutating read:
  - Horizon: the remainder of the current round from `turnCursor.index` onward, **plus exactly one additional full round**, computed by concatenating (a) `turnCursor.queue`'s unresolved tail and (b) a fresh `build_turn_queue(combat)` call against the state as it would stand after the deterministic round-boundary tick (above) — never committed to `turnCursor`, a projection only. One extra round is always sufficient to guarantee every living combatant's next occurrence appears (worst case: a combatant who just acted at the very start of the current round).
  - Repeated occurrences and Motion-inserted extra slots appear exactly as `build_turn_queue()` naturally produces them — no de-duplication.
  - Refreshes (recomputed from scratch, never cached) whenever any input `build_turn_queue()` reads changes — a KO, a `motionTurns`/`motionPower`/`frozenTurns` change, or any other scheduling-affecting mutation. Not persisted in `GameState.state`; re-derived on every `state_changed` while combat is active, like any other read-only projection.
  - Never reveals unresolved-turn outcomes — evade rolls, damage, an enemy's target pick, an ally's heal-vs-attack branch — only who acts and in what order. All of that stays undetermined until an entry actually resolves via `advance_to_next_decision()`.
  - The one-extra-round horizon is the fixed policy, not a per-screen/configurable depth; a future ticket wanting more lookahead amends this bullet.

- **Roster generation** — spawning distinct enemies instead of one scaled blob: `Combat.generate_mugger()`'s `count` (1–3, unchanged roll) now spawns `count` distinct entries off the single "mugger" archetype's base stats (hp 28, atk 4–10), not one `hp × count` blob. `Combat.generate_raid_enemy()`'s `guard_count` (also capped at 3, the squad max) similarly spawns `guard_count` distinct entries, each slot independently rolling a template from `GameData.ENEMY_RAID_GUARDS` (mixed-archetype squads now possible — e.g. one Scrapper + one Vein Guard) unless a caller forces a specific `template_key`, in which case every slot uses that template. Every spawned entry — mugger or guard — gets independent stat variance: `hp`/`attackMin`/`attackMax` each rolled at `base × randf_range(1 − ENEMY_INSTANCE_VARIANCE, 1 + ENEMY_INSTANCE_VARIANCE)` (`Combat.ENEMY_INSTANCE_VARIANCE = 0.15`, **draft, needs balance sign-off**), rounded via `GameState.round_epsilon()` — same-archetype squadmates read as individuals, not clones. Each spawned Territorial Scrapper also rolls a `variant` (visuals only; stats unchanged) via `Rng` from `GameData.TERRITORIAL_VARIANTS` minus the player's own `player.model`, handing out distinct variants within the fight until that pool is used up before repeating; an empty pool gives `"default"`. The combat stage draws the scrapper with that template; save/load and Rewind keep it. **This is a real difficulty increase over today's blob-scaling** (3 full-stat guards vs. one 3×-hp blob) — flagged, not silently absorbed; wants its own playtest pass before shipping to a live encounter.
...
- **Staff block step:** every working cultivator and producer (room hire or founder role) acts at the end of every player time block — `TimeSystem.advance_time_block()` runs `run_staff_block()` for the block that just ended, before any rollover; `TimeSystem.do_rest()` runs it once per block still remaining that day, then rolls over, so a day always yields 3 actions per worker. Order within a block: cultivators, then producers, then (if shared stock grew) Sales' instant delivery re-check via `shared_stock_increased`. A worker acts only if `Payroll.is_working(id)` (a room hire once today's wage is paid; founders unless the business owes them wages). Block output (ore yielded, items crafted, net ore change) accumulates in `morningAccounts.blockProduction` and folds into the next rollover's Morning Brief `production`/`oreMovement`. The rollover staff phase keeps only wages, Sales partial deliveries, settlements/renewals and offer sourcing.
- **Business pot and payday:** `Business.activate()` (Beat 3) sets `potActive`, partners `["archie","james"]`, and Owen's wage entry (`weekly` = constants.json `business.weeklyWages.owen`, 250; `hiredDay` = today). While active, every `Contracts.settle()` payment credits `pot` + `week.receipts` instead of player cash (routed by settle day; Archie lane / faction trade never routed). Each rollover (step ⑥, after settlements) adds 1 to `daysWorked` for every wage not `unpaid`. On a rollover where `day % paydayIntervalDays (7) == 0`: wage due = `round(weekly × daysWorked / 7) + owed`, paid from the pot in full or not at all; a shortfall sets `owed = due`, `unpaid = true`, `promptPending = true` (morning BizBrief "Pay from your own cash?": Yes = `Business.pay_owed_from_cash()`, No = `decline_wage_prompt()`). Remainder R = pot; each partner `floor(R/(partners+1))`, player the rest → cash + bank record "Business share". Ledger record (stable id `payday-N`) appended before cash moves; pot → 0; new week; `daysWorked` → 0. Other rollovers retry `owed` from the pot. Paying owed in full clears `unpaid`. `Payroll.is_working()` is false while `unpaid`.
- **Business stats:** `BusinessStats` tallies into `businessStats.today`: revenue = every `Business.receive()` pot payment; expenses = Sales calc purchases, pot wages (payday and owed retries), owed wages paid from cash, and daily `Payroll` room wages; oreCultivator = staff-block cultivator yield; orePlayer = the player's own `Cultivating.prune()` yield. Rollover step ⑥.4c (after the payday) snapshots the ended day (`world.day − 1`) into `days[]` with `items` = that day's `productionLog` made count — only while `potActive` — then resets `today` and drops days older than `businessStatsDays` (10). BizBrief's Stats tab (shown while `potActive`) charts each metric over the last 10 completed days, zero where no snapshot; its ore chart toggles cultivator/player.
- **Unattended proof:** each contract period carries `periodStartDay`, `playerAssisted` (false) and `delegatedWholePeriod` (= `delegated`), reset by `Contracts.start_period()` on acceptance and every recurring renewal. `set_delegated(false)` clears `delegatedWholePeriod`; delegating on `periodStartDay` sets it. `playerAssisted` is set by the player's own: manual `deliver()`; `Crafting.attempt_craft()` of a requested recipe; `Cultivating.cultivate()`/`prune()` of a vein on any cultivator's list (taints every active period, before the block advances); `Stash.move_*_to_shared()` of a requested type; `Economy.execute_faction_purchase()` of a requested ore. Sales' `buyCalc` purchases never set it. Every settlement stores `qualified` = recurring AND complete AND delegated AND `delegatedWholePeriod` AND NOT `playerAssisted` AND flag `bizA1DelegationUnlocked` (Beat 7) set at settle time; short first periods count.
- **Business Empire questline (Beats 1–8):** `business_empire` objectives in data/objectives.json; side effects in `BusinessQuest`. Beat 1: `player.veins.size() ≥ 2` AND Archie recruited (no Collective gate) → `bizA1Proposed`, Archie pending text `biz_a1_proposition`; checked after every vein-count change (seed, buy, raid claim, self-seed, event completion) and at rollover (⑤i3). The scene sets `bizA1PropositionSeen` + `bizArchieSalesRole`. Beat 2 (`contracts_completed`, minCount 3 → `bizA1MarketProven`): live count of `sales.contractHistory` entries with `settlement.complete`; `Contracts.settle()` refreshes objectives. Starter chain while `bizA1PropositionSeen` and not `bizA1MarketProven`: at most one `biz_starter_*` offer/contract outstanding; `BusinessQuest.maybe_issue_starter()` (after the scene, and rollover ⑥.5b after offer expiry) issues `biz_starter_[starterIndex]` once `day ≥ starterReissueDay`; a starter declined/expired/settled sets `starterReissueDay = day + 1`, and a complete settlement advances `starterIndex`. Starters bypass the random roll; a full pending list defers them. Beat 3: `bizA1MarketProven` → `bizA1OwenIntroQueued`, James pending text `biz_a1_owen` (objective `biz_a1_meet_owen`); checked after `Contracts.settle()`, event completion and rollover (⑧e). The scene unlocks + recruits Owen, sets `bizOwenCultivationRole`, `bizStaffTabOpen` (BizBrief's Staff tab) and `bizA1OwenJoined`, and runs `Business.activate()` (event op `activate_business`), then op `issue_recurring_offers`. Beat 4 (`all_of` objective `biz_a1_apprentice`, active from `bizA1OwenJoined` → `bizA1ApprenticeReady`): Owen `cultivatingSkill ≥ 2` AND `workshop` in `home.rooms`, both live (a tier move wiping the Workshop un-meets it until completion); ToDo shows each as a checklist sub-item. Beat 5: Beat 4 met → `bizA1PartnershipQueued`, James pending text `biz_a1_partnership`; checked after every staff block, a room build, event completion and rollover (⑧f). The scene recruits James (idempotent), sets `craftingSkill` = constants.json `business.jamesJoinCraftingSkill` (5; XP raised to that level's threshold), `bizJamesProductionRole` and `bizA1JamesJoined`. Beat 6: `bizA1JamesJoined` → `bizA1ProductionQueued`, James pending text `biz_a1_production` (objective `biz_a1_production`); checked after event completion and rollover (⑧g); never fires once `bizA1PutToWorkQueued` is set (a save from before Beat 6 existed skips it). The scene teaches the Production role and targets, sets `bizA1ProductionSeen` and runs op `issue_recurring_offers`. Objective `biz_a1_first_order` (`template_periods_completed`, templateId `biz_recurring_time_pearl`, minCount 1 → `bizA1FirstOrderDone`): live count of `sales.contractHistory` entries from that template with `settlement.complete`; ToDo shows "n of 1". Production targets (BizBrief Manage) are settable with an Improved Lab or once any founder may take Production (`Rooms.production_settings_open()`). Beat 7: `bizA1FirstOrderDone` → `bizA1PutToWorkQueued`, Archie pending text `biz_a1_put_to_work` (objective `biz_a1_demo`); checked after `Contracts.settle()`, event completion and rollover (⑧g1). The scene sets `bizA1DelegationUnlocked` (`Contracts.set_delegated(true)` refuses without it; turning off is never gated); it issues no offers. Recurring offers (all weekly, weekday 0): the ore choices `biz_recurring_time_ore` (time ×6) and `biz_recurring_life_ore` (life ×6) are active from `bizA1OwenJoined`; `biz_recurring_time_pearl` (timePearl ×5) from `bizA1ProductionSeen` or `bizA1DelegationUnlocked`; none once `bizA1ProofDone`. `BusinessQuest.maybe_issue_recurring()` (Beat 3 and Beat 6 scenes, rollover ⑥.5c) issues each active one that is not pending or active, once `day ≥ businessQuest.recurringReissueDay[template]`; a declined one sets that to `day + 1`; the two ore choices are not reissued while either runs as a contract; a full pending list defers the rest. An active recurring offer never expires. Beat 7 objective `biz_a1_put_to_work` (`recurring_proof`, minContracts 2, minCrafted 1 → `bizA1ProofDone`): distinct contract ids in `sales.contractHistory` with a `qualified` settlement, ≥ 1 requesting a non-ore line; ToDo shows two checklist rows. `Contracts.settle()` stamps `businessQuest.proofLedgerSize` (ledger length) when first met. Beat 8: at rollover (⑧h) once `business.ledger` has grown past `proofLedgerSize` → `bizA1ClosingQueued`, Archie pending text `biz_a1_closing` whose payload (the scene's context) holds display strings `periodOne`/`periodTwo` (first qualified period of a crafted contract, then another), `receipts`, `owenWage`, `playerShare`, `archieShare`, `jamesShare` from the latest ledger record. An event with `fillFromContext: true` fills `{key}` in card text from its context. The scene sets `bizA1Complete` (act done); pot, payday and staff carry on. Owen's crafting event (not an Act 1 beat): Owen `cultivatingSkill ≥` constants.json `business.owenCraftMinCultivating` (2) AND James recruited AND `workshop` in `home.rooms` → `bizOwenCraftQueued`, James pending text `biz_owen_craft`; checked after event completion and rollover (⑧g2); plays once. The scene sets `bizOwenProductionRole`: Owen may then swap freely between Cultivation and Production (takes effect at the next staff block); while he holds Production his `cultivatorVeins` list is kept but untended. Owen's texts (`OwenTexts`, rollover ⑧g3, pool in data/owen_texts.json): from `bizA1OwenJoined`, a text every `intervalMinDays`..`intervalMaxDays` (2-3) days counted from the join day; each rollover `Payroll.is_working("owen")` is false pushes the due day back 1; a due text waits while the last is unanswered. Pick: an unplayed eligible text at random, else the least recently played; a `needsVein` text is ineligible while Owen's `cultivatorVeins` list has no live vein, else templates `{street}`/`{district}`/`{ore}` from a random one. Replies (2-3) are tapped in Messages; a question's one `correct` reply grants `cultivatorActionXp` cultivating XP (skill cap applies); Owen answers every reply.
- **Producer (per block):** within the block's staff step, producers repeatedly attempt the first recipe in priority order that is unlocked, below its effective target, and affordable from shared ore, until every target is met or nothing affordable remains; then they wait for the next block. Each attempt uses the CONTACT's skill in the §3.5 formulas (workshopBonus included), consumes player ore (failures too), and awards contact XP (full/⅓). Multiple producers take turns one attempt at a time in `Contacts.contacts_in_role("production")` order, re-checking live stock before each attempt, so a shared target is never double-counted. Safety cap `Rooms.MAX_PRODUCER_ATTEMPTS_PER_BLOCK` (10000) total attempts per block. Effective target = `labThresholds[recipe]` (personal inventory target), plus — when `labCoverContracts[recipe]` is on (ticket 30) — the summed undelivered quantity of every active contract requesting that item (a recurring contract's next period doesn't exist in state until settlement creates it, so this already excludes any future period). Additive, personal-target-first-reserved: Sales (§3.10 below / Contracts._shared_stock) may only deliver from the contract-need portion of stock, never the personal-target reserve. Priority order: the recipe with the highest-priority (lowest sales.priorityOrder index) covered, unmet contract need first; every other recipe follows in `labThresholds`' own key-insertion order.
- **Cultivator (per block, hold-at-target):** each vein on a cultivator's `cultivatorVeins` list holds a target in `veinStationTargets[veinId]` (default 70 on assignment). Each block the cultivator takes the one vein furthest outside `target ± 5` (`Rooms.pick_vein()`; ties to list order): above → prune down to the target (§2.4 yield formula, ore into shared stock); below → one cultivate roll at the contact's own skill; every vein in band → idle. `cultivatorActionXp` (constants.json, 2) cultivating XP per action, success or fail.
