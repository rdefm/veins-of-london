...
- **`combat.locationKey: String`** — recorded once at `_start_combat()`, immutable for the fight. Derivation by `context`: `CONTEXT_RAID`/`CONTEXT_DEFEND_VEIN`/`CONTEXT_EVENT_RAID` → the fought vein's `district` (§2.1, via `veinId`); `CONTEXT_MUGGING`/`CONTEXT_EVENT_MUGGING`/`CONTEXT_ARCHIE_DEAL_MUGGING` → `world.currentDistrict` at combat start; `CONTEXT_HOME_RAID` → the fixed key `"home"` (home isn't a district). Backdrop lookup order using this key: `docs/combat-animation-vision.md` §2.1.
