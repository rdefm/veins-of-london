...
**Terroir**:
The land itself — a site's tier (`poor`/`fair`/`rich`/`saturated`, driving `terroirYieldMult` directly) and its discovery bonuses — carries all of a vein's long-term progression now that growth is never permanent. Which sites you hold matters far more than any one vein's history. `data/sites.json`'s `discoveryBonusPool` is `["vigour", "wildCeiling", "yield"]`: `vigour` adds +1 to rightward drift and −1 to leftward drift (min 0, stacks with the King's Cross district special, which is the same effect); `wildCeiling` raises `Cultivating.ceiling(vein)` from 100 to 120; `yield` boosts the rolled prune amount. Landed in vein-growth-state tickets 01 (ceiling plumbing) and 05 (the rename and the effects themselves).
