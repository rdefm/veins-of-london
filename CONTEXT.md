...
Every vein also carries a persistent `level: int` (1..`Cultivating.level_cap_for_tier(tier)`: poor 2 / fair 3 / rich 4 / saturated 5), seeded at 1 on creation and unrelated to `growth`/`value_tier()`. It scales nightly drift magnitude (`level + rand(1,5)`, R§1.2/§8.3) and, from cultivation-refining ticket 02 onward, blends into the same combined-magnitude seam `value_tier()` feeds today. Ways to raise or lose it land in later cultivation-refining tickets (05/06/07).
