...
func run() -> void:
	run_case("new_game_state_matches_schema_defaults", func():
		...
		assert_eq(s["player"]["hp"], 40, "player.hp")
		assert_eq(s["player"]["hpMax"], 40, "player.hpMax")
		assert_eq(s["player"]["attackMin"], 3, "player.attackMin")
		assert_eq(s["player"]["attackMax"], 7, "player.attackMax")
