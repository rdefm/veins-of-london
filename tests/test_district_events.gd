...
func run() -> void:
	...
	run_case("lucky_omen_consumed_on_next_sale_and_can_apply_a_10pct_bump", func():
		var seed := _find_seed_for(500, func():
			...
			return not result.get("mugged", true) and GameState.state["player"]["cash"] > 40 + int(floor(180 * 0.5))
		)
		...
		# time basePrice 60 * 1.10 = 66/unit, qty 3 -> gross 198, cut floor(99) = 99
		assert_eq(GameState.state["player"]["cash"], 40 + 99, "the +10% bump should be reflected in the payout")
	)
