...
func run() -> void:
	run_case("apps_lists_the_seven_existing_apps_in_a_fixed_order", func():
		var ids: Array[String] = []
		for app in PhoneApps.apps():
			ids.append(app["id"])
		assert_eq(ids, ["messages", "notes", "factions", "ticker", "profile", "saveload", "notifications"], "grid slot order comes straight from the registry order")
	)
