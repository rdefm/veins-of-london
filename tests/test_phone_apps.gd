...
func run() -> void:
	run_case("apps_lists_the_eleven_existing_apps_in_a_fixed_order", func():
		GameState.reset()
		var ids: Array[String] = []
		for app in PhoneApps.apps():
			ids.append(app["id"])
		assert_eq(ids, ["bizbrief", "notes", "factions", "ticker", "profile", "saveload", "notifications", "bank", "property", "contacts", "vfl"], "grid slot order comes straight from the registry order")
	)
