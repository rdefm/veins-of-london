...
func run() -> void:
	run_case("property_shows_the_current_tiers_stats", func():
		...
		assert_true(_label_texts(phone).has(expected), "current tier's stats line matches bedsit's daily cost/raid risk/rooms")
	...
	run_case("property_shows_the_next_tier_up_with_its_own_stats_and_upgrade_cost", func():
		...
		assert_true(_find_button(phone, "Move for £1200") != null, "upgrade action shows flat's upgradeCost")
	...
	run_case("property_upgrade_button_is_disabled_without_enough_cash_and_calls_upgrade_tier_when_enabled", func():
		...
		poor_phone._ready()
		var poor_button := _find_button(poor_phone, "Move for £1200")
		...
		assert_true(_label_texts(poor_phone).has("Not enough cash."), "shows the reason it's disabled")
		poor_phone.free()

		GameState.state["player"]["cash"] = 5000
		var rich_phone := PhoneScreen.new()
		rich_phone._ready()
		var rich_button := _find_button(rich_phone, "Move for £1200")
	...
	run_case("property_shows_a_max_tier_message_and_no_upgrade_button_at_the_top_tier", func():
		...
		assert_true(_label_texts(phone).has("Top of the ladder. Nowhere further to move."), "max-tier message renders")
		assert_true(_find_button(phone, "Move for £150000") == null, "no upgrade button at the top tier")
