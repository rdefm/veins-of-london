...
func run() -> void:
	...
	run_case("messages_conversation_root_is_freed_when_the_shell_rebuilds_for_another_app", func():
		...
		assert_true(root != null and root.get_parent() == phone, "conversation root mounts on the shell")
		assert_true(not phone._content.get_parent().visible, "shared scroll body hidden while a conversation is open")
		PhoneNav.go_home()
		...
		assert_true(phone._content.get_parent().visible, "shared scroll body visible again on home")
		phone.free()
	)
