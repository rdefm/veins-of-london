...
func run() -> void:
	...
	run_case("trade_sheet_has_fixed_footer_and_category_tabs", func():
		...
		assert_true(view.visible and not layer._card.visible, "trade uses its own sheet")
		assert_true(_find_cost_button(view, "Ore") != null)
		assert_true(_find_cost_button(view, "Items") != null)
		assert_true(_find_cost_button(view, "Veins") != null)
	...
	run_case("items_bucket_available_tiers_and_keep_selection_visible_when_collapsed", func():
		GameState.reset()
		...
		group.pressed.emit()
		assert_true(_find_button_prefix(view, "Blast").accessibility_name.contains("2 selected"), "collapsed group keeps its selection count")
		assert_true(not NodeQuery.label_texts_with_symbols(view).has("Tier 2"), "tier rows hide when collapsed")
		_find_cost_button(view, "Review trade →").pressed.emit()
		assert_true(NodeQuery.label_texts_with_symbols(view).any(func(t: String): return t.contains("Blast · tier 2")))
		assert_true(NodeQuery.label_texts_with_symbols(view).any(func(t: String): return t.contains("Blast · tier 3")))
		layer.free()
	)

	run_case("ore_and_vein_rows_use_the_map_ore_silhouettes", func():
		GameState.reset()
		...
		layer._ready()
		var view: PanelContainer = layer._trade_view
		var glyphs := view.find_children("", "SymbolGlyph", true, false)
		assert_eq(glyphs.size(), 1, "only stocked time ore appears on the sell tab")
		assert_eq((glyphs[0] as SymbolGlyph).symbol, "", "empty symbol forces the Map vector fallback")
		assert_true((glyphs[0] as SymbolGlyph).draw_fallback.is_valid())
		_find_cost_button(view, "Veins").pressed.emit()
		glyphs = view.find_children("", "SymbolGlyph", true, false)
		assert_eq(glyphs.size(), 1)
		assert_eq((glyphs[0] as SymbolGlyph).symbol, "")
		_find_cost_button(view, "+ Add").pressed.emit()
		assert_eq(GameState.state["sellState"]["vein_v1"], 1)
		assert_true(NodeQuery.label_texts_with_symbols(view).has("%d%% base chance of mugging" % int(round(Economy.MUG_BASE_CHANCE_VEIN * 100.0))))
		layer.free()
	)
	...
	run_case("faction_buy_rows_show_stock_and_disable_when_sold_out", func():
		...
		_find_cost_button(view, "Buy from contact").pressed.emit()
		var sold_out := 0
		for label in NodeQuery.label_texts(view):
			if label == "Sold out":
				sold_out += 1
