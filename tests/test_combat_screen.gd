...
func run() -> void:
	...
	run_case("stage_backdrop_spans_the_full_stage_with_no_inset", func():
		...
		assert_true(screen._stage._backdrop_texture.texture != null, "mugging ships a real plate")
		var min_size: Vector2 = screen._stage._backdrop_texture.get_combined_minimum_size()
		assert_true(min_size.x <= stage_size.x and min_size.y <= stage_size.y, "the plate's native size must not grow the rect past the stage (would clip as a zoom-in): %s" % str(min_size))
